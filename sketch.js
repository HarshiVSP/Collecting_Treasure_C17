...
function spawnCash() {
  if (frameCount % 200 === 0) {
    var cash = createSprite(350, 40, 10, 10);
    cash.addImage(cashImg);
    cash.scale = 0.12;
    cash.velocityY = 3;
    cash.x = Math.round(random(50, 300));
    cash.lifetime = 150;
    cashG.add(cash);
  }
}
...
function spawnDiamonds() {
  if (frameCount % 320 === 0) {
    var diamonds = createSprite(350, 40, 10, 10);
    diamonds.addImage(diamondsImg);
    diamonds.scale = 0.03;
    diamonds.velocityY = 3;
    diamonds.x = Math.round(random(45, 250));
    diamonds.lifetime = 150;
    diamondsG.add(diamonds);
  }
}

function spawnjewelry() {
  if (frameCount % 410 === 0) {
    var jewelry = createSprite(350, 40, 10, 10);
    jewelry.addImage(jewelryImg);
    jewelry.scale = 0.13;
    jewelry.velocityY = 3;
    jewelry.x = Math.round(random(60, 320));
...
function spawnSword() {
  if (frameCount % 530 === 0) {
    var sword = createSprite(350, 40, 10, 10);
    sword.addImage(swordImg);
    sword.scale = 0.1;
    sword.velocityY = 3;
    sword.x = Math.round(random(75, 280));
    sword.lifetime = 150;
